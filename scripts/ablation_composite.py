#!/usr/bin/env python3
"""Ablation experiment: Step6 vs Step6+8composite on R_40_10_1.

Two configurations:
  1. step6:    Step6=on,  Step8=off (best known config)
  2. step6+8c: Step6=on,  Step8=composite (sortie + polish + reanchor as atomic move)
...
CONFIGS = {
    "step6":    {"step6": True, "step8": False},
    "step6+8c": {"step6": True, "step8": True},
}
...
def run_once(instance, evaluator, config, seed, flags):
    ...
    sa_config["drone_sortie_max_customers"] = 2
    sa_config["drone_sortie_top_k"] = 3
    sa_config["drone_sortie_max_sorties"] = 5
...
def main():
    ...
    out = Path("results/ablation_composite.json")
