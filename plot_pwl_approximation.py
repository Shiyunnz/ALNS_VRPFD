...
C = math.sqrt(g**3 / (2.0 * rho * sigma * h)) / 1000.0  # kW 系数

def power_func(omega):
    return (W + m + omega) ** 1.5 * C

omega_max = 10.0  # 典型无人机最大载荷 kg
K_energy = 3      # YAML 配置的分段数
...
# ===================================================================
# 2. 剥夺成本函数  f(τ) = exp(1.5031 + 7.032τ) − exp(1.5031)
# ===================================================================
def delay_func(tau):
    return math.exp(1.5031 + 7.032 * tau) - math.exp(1.5031)

max_delay = 3.0   # 小时
K_delay = 3        # YAML 配置的分段数
...
# --- (d) 剥夺成本近似误差 ---
ax = axes[1, 1]
# 只显示 τ > 0.05 区域的相对误差（避免接近零时的数值噪声）
mask = tau_cont > 0.05
