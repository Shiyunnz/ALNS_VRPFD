...
默认遍历 Instance25 (25 客户点) 算例集，并考察不同无人机绝对速度对以下指标的影响：
1. 总成本 (Total Cost)
2. 相对基线成本节省百分比 (Cost Saving vs Baseline %)
3. 无人机服务的客户点数量 (Drone-served Customers)

统计口径：
- 每个 (instance, drone_speed) 运行 k 次（--trials）
...
# 绝对无人机速度值 (km/h) 用于实验横坐标
# 基准速度为 70.0 km/h（用于计算baseline），其他值用于测试
DRONE_SPEED_LEVELS = [70.0, 80.0, 90.0, 100.0, 110.0, 120.0, 130.0, 140.0, 150.0]
BASELINE_DRONE_SPEED = 70.0
...
OUTPUT_DIR = Path(__file__).parent / "results_new" / "drone_speed"
OUTPUT_CSV = OUTPUT_DIR / "drone_speed_sensitivity_results.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="单算例模式下指定算例名或路径，例如 R_40_25_2 或 data/Instance25/R_40_25_2.txt",
    )
    parser.add_argument(
        "--drone-speeds",
        type=str,
        default=None,
        help="无人机速度水平（km/h），逗号分隔，例如 '40.0,50.0,70.0,80.0'。如果不指定，使用默认列表。",
    )
    parser.add_argument(
        "--skip-baseline",
        action="store_true",
        help=f"跳过运行基线（{BASELINE_DRONE_SPEED:.1f} km/h）实验，假设已运行并从 CSV 读取。",
    )
    parser.add_argument(
        "--append",
        action="store_true",
...
def run_single_experiment(
    ...
    *,
    ...
    print(
        f"  Running: drone_speed={drone_speed:.2f}, same_truck={same_truck_retrieval}")
    ...
    if 'drone' in instance.vehicle_specs:
        ...
        instance.vehicle_specs['drone'].speed = drone_speed
    ...
    destroy_ops, repair_ops = build_operators(
        ...
        # Using 'all' or 'new' as per preference? run_alns uses 'all' by default in main_alns, or 'new'.
        repair_set="all",
        # The user said "use run_alns logic". run_alns.py main_alns uses repair_set="all".
    ...
    with open(OUTPUT_CSV, "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if instance not in instance_paths:
                continue
            try:
                drone_speed = float(row.get("drone_speed", 0))
                cost = float(row.get("best_cost", "inf"))
            except (TypeError, ValueError):
                continue
            if math.isclose(drone_speed, BASELINE_DRONE_SPEED):
...
def run_drone_speed_sensitivity_analysis(
    instance_paths: List[str],
    drone_levels: List[float] | None = None,
    skip_baseline: bool = False,
    trials: int = 5,
) -> List[Dict[str, Any]]:
    """运行无人机速度敏感度分析并返回结果列表。"""

    if drone_levels is None:
        drone_levels = DRONE_SPEED_LEVELS

    print("=" * 80)
    print("无人机速度敏感度分析 (Best of N)")
    print("=" * 80)
    print(f"测试算例: {len(instance_paths)} 个")
    print(f"无人机速度水平: {drone_levels}")
    print(f"Baseline Drone Speed: {BASELINE_DRONE_SPEED}")
    ...
    # 1. 预先运行 Baseline 获取基准 Costs 或从 CSV 读取
    print(
        f"\nPhase 1: Handling Baseline (Drone Speed={BASELINE_DRONE_SPEED})...")
        ...
        for idx, instance_path in enumerate(instance_paths, 1):
            ...
            for t in range(trials):
                ...
                res = run_single_experiment(
                    ...
                    drone_speed=BASELINE_DRONE_SPEED,
    ...
    # 2. Iterate through Drone Speeds and run/process
    print(f"\nPhase 2: Running Sensitivity Tests...")

    for drone_speed in drone_levels:
        print(f"\n>>> Testing Drone Speed: {drone_speed:.2f} <<<")

        current_level_results = []

        for idx, instance_path in enumerate(instance_paths, 1):
            instance_name = Path(instance_path).stem
            # If this is the baseline drone speed, use cached result
            if math.isclose(drone_speed, BASELINE_DRONE_SPEED):
        ...
        # Compute Averages for this Drone Speed Level
        valid_savings = [r["cost_saving_vs_baseline"] for r in current_level_results if math.isfinite(
            r.get("cost_saving_vs_baseline", math.nan))]
        valid_drones = [r["best_drone_customers"] for r in current_level_results if math.isfinite(
            r.get("best_drone_customers", math.nan))]
        ...
        print(f"--- Summary for Drone Speed {drone_speed:.2f} ---")
...
def write_summary_csv(results: Iterable[Dict[str, Any]], out_path: Path) -> None:
    """Write a compact summary CSV grouped by scale and drone_speed.
    Updates existing file if it exists, replacing rows with same scale/drone_speed.
    """
    # 1. Collapse to best-of-k per (instance, drone_speed)
    by_instance_level: dict[tuple[str, float], list[Dict[str, Any]]] = defaultdict(list)
    for r in results:
        inst = r.get("instance")
        speed = r.get("drone_speed")
        if not isinstance(inst, str) or not isinstance(speed, (int, float)):
            continue
        by_instance_level[(inst, float(speed))].append(r)
    ...
    for row in best_rows:
        ...
        speed = row.get("drone_speed")
        best_cost = row.get("best_cost")
        if not isinstance(inst, str) or not isinstance(speed, (int, float)):
            continue
        if not isinstance(best_cost, (int, float)) or not math.isfinite(best_cost):
            continue
        if math.isclose(float(speed), BASELINE_DRONE_SPEED):
            baseline_by_instance[inst] = float(best_cost)

    # Fallback: if configured baseline speed is missing for an instance, use minimum speed level found.
    ...
    for inst, rows in rows_by_instance.items():
        ...
        candidate_rows = [
            r for r in rows
            if isinstance(r.get("drone_speed"), (int, float))
            and isinstance(r.get("best_cost"), (int, float))
            and math.isfinite(float(r.get("best_cost")))
        ]
        if not candidate_rows:
            continue
        candidate_rows.sort(key=lambda r: float(r.get("drone_speed")))
    ...
    for r in best_rows:
        ...
        if not isinstance(speed, (int, float)):
            continue
        grouped[(scale, float(speed))].append(r)

    new_rows = []
    keys_to_update = set()

    for (scale, speed), items in sorted(grouped.items()):
        ...
        new_rows.append(row)
        keys_to_update.add((str(scale), float(speed)))
        ...
        try:
            with out_path.open("r", newline="", encoding="utf-8") as f:
                ...
                if reader.fieldnames:
                    # check if headers match roughly, if not, might be corrupt/old format, overwrite safely?
                    # For now assume format is stable
                    for row in reader:
                        # Convert types for comparison
                        try:
                            r_scale = str(row["scale"])
                            r_speed = float(row["drone_speed"])
                            if (r_scale, r_speed) not in keys_to_update:
                                existing_rows.append(row)
                        except (ValueError, KeyError):
                            # Malformed row, skip or keep? Let's keep if we can't parse to be safe, or drop.
                            # Dropping is safer to avoid accumulation of garbage.
                            pass
    ...
    # Sort by scale then drone_speed
    all_rows.sort(key=lambda x: (str(x.get("scale", "")),
                  float(x.get("drone_speed", 0))))
...
def main() -> None:
    ...
    # Parse drone speeds if provided
    drone_levels = None
    if args.drone_speeds:
        try:
            drone_levels = [float(x.strip())
                            for x in args.drone_speeds.split(',')]
        except ValueError:
            print("Error: Invalid drone speeds format. Use comma-separated floats.")
            return
    ...
    results = run_drone_speed_sensitivity_analysis(
        instance_paths, drone_levels, skip_baseline=args.skip_baseline, trials=args.trials)
    write_results(results, append=args.append)
    # Extra summary CSV for plotting: avg cost saving vs baseline per scale & drone_speed
    summary_path = OUTPUT_DIR / "drone_speed_summary.csv"
