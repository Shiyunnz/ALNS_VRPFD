#!/usr/bin/env python3
"""Plot drone speed sensitivity as a single chart (battery/drone-count style)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot drone speed sensitivity (single figure)")
    parser.add_argument(
        "--scale",
        type=str,
        default="Instance25",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom summary CSV path (default: results_new/drone_speed/drone_speed_summary.csv)",
    )
    parser.add_argument(
        "--output",
        type=str,
...
def main() -> int:
    ...
    summary_csv = (
        ...
        if args.summary_csv
        else sensitivity_dir / "results_new" / "drone_speed" / "drone_speed_summary.csv"
    )
    output_pdf = (
        Path(args.output)
        if args.output
        else sensitivity_dir / "results_new" / "drone_speed" / "drone_speed_sensitivity_plot_large.pdf"
    )
    ...
    data = df[df["scale"] == args.scale].copy().sort_values("drone_speed")
    if data.empty:
    ...
    # Match battery / drone-count plotting style
    bar_red_fill = "#F6D8E6"
    ...
    x_vals = data["drone_speed"]
    y_cost = data["avg_cost_saving_vs_baseline"]
    y_drone = data["avg_best_drone_customers"]

    ax1.set_xlabel("Drone Speed (km/h)", fontsize=14, fontweight="bold")
